...
# |  Harvard Lines  |
# \_________________/
# Well-balanced English test lines used for generating speech with TTS engines
#   Intended to be used to prepare lines for zero-shot TTS engines
#   Also works well for general testing of TTS engines
# They come form here and there is no copyright notice, so I think it's fair to include them in this file:
#   https://www.cs.columbia.edu/~hgs/audio/harvard.html
# Changed some of these sentences to spell words phoenetically: 'read' --> 'reed' and 'tear' --> 'tare'
# Also removed several lines that consistently resulted in flubs, particularly those that had the word 'slash' in them
#  Most TTS engines badly fail on that word, though I've never found a reason why
...
Reed verse out loud for pleasure.
...
Lyve wires should be kept covered.
...
Thick glasses helped him reed the print.
...
It matters not if he reeds these words or those.
...
Tare a thin sheet from the yellow pad.
...
The gloss on top made it unfit to reed.
...
# Provides count Harvard lines, as a string, with each line separated by a single space
def harvard_lines(count = 3)
    unless count.is_a?(Integer)
        raise "count MUST be an Integer!"
    end
    
    # Ruby is awesome for text manipulation!
    # Love the fact this is clear and simple to understand one-liner
    return HARVARD.split("\n").shuffle!.slice!(0, count).join(" ")
end
